...
mkts = ['agrs',	'incnt', 'sbd', 'bsd', 'golos', 'pdc', 'grc','start','clam', 'crb', 'enrg','gbg','geo',
		'spr','swift','adt','thc','bay','aby','2give','synx','vtr','fldc','dope','bcy', 
		'gld',	'ebst','dtb',	'bitb','pot',	'vrm',	'meme','slr','vox','omni','sphr','iop',
		'gno','kore','zcl',	'xst','btcd','sib',	'dgd','swt','ftc','apx','excl','coval',
		'mona','rep','part','gup','blitz','trust','xaur','time','tks','ioc',	'xdn','neos',
		'nxc','xmy','xmg','bta','flo','ptc','nlg','zen','xwc','myst','emc2','pink','amp',	
		'hmq','byc','emc','cann',	'ppc','dar','music','lgd','trst',	'cfi','qwark','fun',
		'ion','ant','via','mue','nbt','rads','cpc','erc','sngls','crw','qrl','doge',
		'dmd','block',	'nmr','fct','bnt','dct','mln','safex',	'xcp','xlm','maid','dcr',	'steem',
		'blk','vtc','lbc','lmc','cloak','ardr',	'xzc','infx','tkn','pivx','shift','storj',
		'nxs','game','rdd','grs','ubq','gbyte',	'sc',	'vrc','rlc','gnt','adx','xem','nxt','burst','exp']


bit = Bittrex("6b0206e882424f96a255ba813169a3e0", "bafe63c8f85b4abbabcd86df1e8531ff")
test = pd.DataFrame(bit.get_market_history('btc-ebst')['result'])

def gmt():
	time = datetime.datetime.now() - pd.Timedelta('5 hours 30 minutes')
	return time

#parallelly pulling data
def hist_1(mkt):
  print mkt
  his1 = pd.DataFrame(bit.get_market_history('btc-%s'%mkt)['result'])
  his1['TimeStamp'] = pd.to_datetime(his1['TimeStamp'])
  his1['TimeElapsed'] = gmt() - his1['TimeStamp']
  his1 = his1.sort_values('TimeStamp', ascending = False)
  his1['coin'] = mkt
  return his1
...
hstry = Parallel(n_jobs= 138, backend = 'threading')(delayed(hist_1)(i) for i in mkts) #4.4secs
b = datetime.datetime.now()
print b-a


def history(mkt, his_past):
  print mkt
  his_present = pd.DataFrame(bit.get_market_history('btc-%s'%mkt)['result'])
  his_present['TimeStamp'] = pd.to_datetime(his_present['TimeStamp'])
  his_present = his_present.sort_values('TimeStamp', ascending = False)
  his_present['TimeElapsed'] = gmt() - his_present['TimeStamp']
  his_present['coin'] = mkt
  his = (his_present.append(his_past)).drop_duplicates('Id')
  return his

'''
def coin_picker_test(data):
	for i in data:
		print i.iloc[0,8]
		if len(i) >= 210:
			pick = i.iloc[0,8]
...
sec_7 = pd.Timedelta('7 seconds')
sec_15 = pd.Timedelta('15 seconds')
sec_30 = pd.Timedelta('30 seconds')

def timedelta(time):
	return pd.Timedelta('%d seconds'%time)


def pct_change(data, time):
	data_time = data.loc[data['TimeElapsed'] <= timedelta(time)]
	data_time.reset_index(drop = True, inplace=True)
	if len(data_time) != 0:
		pct_time = (data_time['Price'][0] - data_time['Price'].min())/data_time['Price'].min()
	else:
		pct_time = 0
	return pct_time


def coin_picker(data,p1,p2,p3,t1,t2,t3):
	for i in data:
		buy = i.loc[i['OrderType'] == 'BUY']
		buy.reset_index(drop = True, inplace=True)
		pct_p1 = pct_change(buy, t1)
		pct_p2 = pct_change(buy, t2)
		pct_p3 = pct_change(buy, t3)
		if pct_p1 >= p1 and pct_p2 >= p2 and pct_p3 >= p3:
			pick = i.iloc[0,8]
			sell = i.loc[i['OrderType'] == 'SELL']
...
a = datetime.datetime.now()
with Parallel(n_jobs=138, backend = 'threading') as parallel:
	coin = ''
	sp = 0
	n_iter = 0
	while coin == '':
		try:
			hstry = parallel(delayed(history)(mkts[j], hstry[j]) for j in range(len(mkts)))
			coin, sp = coin_picker(hstry,0.2,0.02,0.03,5*60*60,5*60*60,5*60*60)
		except:
			pass
		n_iter = n_iter + 1
